...
async function diagnose() {
  try {
    ...
    const teacherIds = await prisma.teachingArrangement.findMany({
    ...
    teachers.forEach(t => {
      ...
      console.log(`  ${t.name} (id=${t.id}): textbookIds=${JSON.stringify(tbIds)}, length=${tbIds.length}`);
    });
    
    console.log('\n=== 2. 班级教材数据 ===');
    const classIds = await prisma.teachingArrangement.findMany({
      where: { courseId, semester },
      select: { classId: true }
