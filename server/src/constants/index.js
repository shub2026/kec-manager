...
// 固有班级延续配置（2026-08 新增）
// 目标：排课时让教师优先回到上学期教过的班级（固有班级），保持教学连续性
// 软性优先：仅在评分与班级拿取顺序上加权，不改变容量/意向/教材等硬约束，
// 上学期教过该班的教师容量满或意向不匹配时，班级照常排给其他教师
// 关闭时（ENABLED=false，默认）行为与无此功能完全一致，且不查询上学期数据
export const INHERENT_CLASS = {
  ENABLED: false, // 静态默认开关：是否启用固有班级延续（system_settings key='inherent_class_enabled' 可动态开启）
  CONTINUITY_WEIGHT: 8, // 延续奖励分值：教师上学期教过该班级时的评分加分
  // 权重取舍说明：设 8 明显高于学院/层次意向（各 5），使延续成为常规首选因素；
  // 但远低于教材强惩罚（-300），确保"一本教材"内聚原则不被延续性破坏
};
