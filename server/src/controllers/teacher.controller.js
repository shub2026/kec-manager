...
export async function deleteTeacher(req, res, next) {
  try {
    ...
    // 检查是否有教学安排
    const assignmentCount = await prisma.teaching_assignments.count({ where: { teacher_id: Number(id) } });
...
export async function batchUpdateDefaultHours(req, res, next) {
  try {
    ...
    const hours = default_weekly_hours != null && default_weekly_hours !== '' ? Number(default_weekly_hours) : null;
