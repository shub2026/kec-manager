...
- **清空可空字段未落库（教材/班级/教师/小程序）**：表单清空出版社等可空字段时前端吐 `undefined`，被 `JSON.stringify` 丢键，后端将“键缺席”视为“本次不修改”，导致旧值保留但接口仍报成功。现统一改为显式发 `null`（缺席=不修改，`null`=清空），并在 `App.vue` 全局注入 `value-on-clear=() => null`，防止未来新增 clearable 控件复发
- **定价 0 被存成 null**：教材创建/更新时 `price ? Number(price) : null` 会把合法的 0 元定价误判为空值，改用显式 `null/undefined/''` 判断
- **后端空值口径归一**：`express-validator` 的 `.trim()` 会把 `null` 归一为 `''`，控制器新增 `normalizeNullableText` 统一落回 `null`，与导入链路口径一致；`price` 校验改为 `optional({ nullable: true })` 以放行 `null`

### 测试

- 新增教材编辑弹窗前端用例、班级 composable 用例、教材控制器用例，以及教材路由 HTTP 全链路集成测试（真实中间件链 + 真实 `buildUpdateData`）
- 服务端全量 81 个测试文件 1943 用例、前端全量 49 个测试文件 530 用例通过

## [1.22.0] - 2026-09-01

### 新增

- **文档资料页面（系统管理）**：超级管理员专属的文档管理页，支持 Word/Excel/PDF/JPG 的上传、下载、重命名、删除；平铺列表 + 文件名关键词搜索 + 类型分组筛选 + 分页；文件二进制存 `UPLOAD_DIR/documents/` 磁盘目录，SQLite 仅存元数据（新增 `documents` 表与迁移）
- **上传安全校验**：扩展名白名单 + 文件头魔数校验（ZIP/OLE2/PDF/JPEG 签名）+ 下载路径防穿越 + 上传限流；上传/下载/重命名/删除全部写审计日志
- **内容去重**：上传时流式计算 SHA-256 存入 `documents.file_hash`，内容相同的文档拒绝重复入库并提示已存在的文件名

### 变更

- **重命名交互优化**：重命名弹窗仅显示基础名，扩展名以只读后缀展示并自动保留，防止误改文件类型
- **文档列表列宽优化**：改为“固定列 + 弹性列”混合策略，短内容列锁定宽度，文件名列吸收剩余宽度

### 修复

- **中文文件名乱码**：修复 multipart 上传时 busboy 按 latin1 解析导致中文文件名乱码的问题（启发式 UTF-8 还原）

### 测试

- 服务端全量 79 个测试文件 1899 用例、前端全量 47 个测试文件 490 用例通过

## [1.17.2] - 2026-08-26

### 新增

- **方案查询页方案概览条**：选中方案后在课程矩阵上方展示方案名称（含版本）、专业与培养层次标签，以及学院、公共/专业课数、课程数、总课时指标，信息结构与培养方案明细页概览条一致；数据全部复用方案列表与课程接口，无新增请求；课程加载中指标显示占位符避免 0 值闪动，窄屏下指标自然换行并隐藏分隔符

### 变更

- **侧边栏浅色化改造**：侧边栏由深色渐变改为浅色（冷蓝白底 + 右侧细分隔线），消除白色透明材质硬编码设计债；激活态调整为满宽浅蓝底 + 品牌蓝字加粗 + 左侧 3px 竖条，hover 用中性浅灰，与内容区同源中性色形成“安静外壳”
- **全局中性色注入品牌蓝调**：边框（#e3eaf3 / #c9d7ea）与表头、hover 底色（#eff4fa）在 slate 基础上增强蓝调，与品牌蓝建立色温血缘关联；侧边栏底色同步调整为冷蓝白，消除暖白/冷白色温冲突
- **首页概览配色收敛**：排课进度卡 32px 大数字由深绿改为深色中性（绿色语义仅保留在进度条/标签/状态条等小面积元素）；教师环图绿/橙/灰三色降饱和约 20%（色相不变，与人员页标签联想保持）；欢迎区装饰点阵白渐变硬编码令牌化
- **侧边栏布局优化**：修复折叠态 logo 区溢出导致展开按钮被裁剪的问题（折叠态改竖排布局）；菜单项行高由 56px 收至 40px（桌面/抽屉/折叠弹出菜单三处统一）；折叠按钮升级为 24×24 触达区并增加 hover 反馈
- **全站排版规范化**：卡片标题统一 16px/600（与设置页同规格）；表格正文/表头字号显式锁定 14px/12px 防升级漂移；约 100 处 12/13/14px 硬编码字号替换为 --font-size-* 令牌（图标尺寸与移动端收缩值有意保留）

### 测试

- 前端全量 33 个测试文件 286 用例通过（本次为纯前端改动，服务端未变更）
