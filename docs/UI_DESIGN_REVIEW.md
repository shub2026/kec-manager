...
| 优先级 | 改动项 | 预期效果 | 工作量 | 状态 |
|--------|--------|---------|--------|------|
| P0 | 替换主色 + 侧边栏配色 | 立刻脱离"默认模板"感 | 小 | ✅ v1.0.0 |
| P0 | Dashboard 欢迎区渐变背景 + 去掉底部介绍 | 首屏质感大幅提升 | 小 | ✅ v1.0.0 |
| P1 | 页面切换动画 + 活跃菜单指示条 | 交互品质感 | 小 | ✅ v1.0.0 |
| P1 | 表格操作列改 hover 浮动 + 自定义空状态 | 减少视觉噪音 | 中 | ✅ v1.0.0 |
| P2 | 统计数字动画 + sparkline 图表 | 数据展示升级 | 中 | ✅ v1.0.0 |
| P2 | Page Header 独立 + 布局差异化 | 页面层次感 | 中 | ✅ v1.1.0（推广至全部 15 个页面） |
| P3 | 骨架屏推广到所有列表页 | 加载体验统一 | 小 | ✅ v1.0.0 |
| P3 | 表单提交动画反馈 | 操作确认感 | 小 | 待做 |

> 附：v1.1.0 额外实施了第二轮架构审查（`UI_ARCHITECTURE_REVIEW_2.md`）的全部 13 项建议，包括全局样式修正、composable 健壮性加固、登录页视觉优化等。
