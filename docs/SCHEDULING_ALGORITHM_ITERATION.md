...
仅 `auto-arrange.js` 中的算法核心，约 200-300 行新增代码：

| 现有组件 | 改动方式 |
|---------|---------|
| 五阶段分配（Phase 1-5） | **保留不动**，作为初始解构造 |
| `trySwapUnassigned()` | 被邻域搜索算子替代（功能升级） |
| `calcMatchScore()` | 从"选教师评分"扩展为"全局目标函数" |
| `selectBestTeacher()` | 被禁忌接受准则替代 |
| `assignRound()` 兜底 | 保留用于初始解构造，后续由禁忌搜索优化 |
...
- 可通过配置开关（如 `system_settings` 中新增 `arrange_optimization_mode`）控制是否启用禁忌搜索
- 关闭时行为与当前完全一致，零风险回退
- 超时保护：单课程 10 秒上限，超时则返回贪心初始解
