...
### 五、分层选择机制（selectBestTeacher）

在课时约束通过后，系统使用**分层选择**而非简单分数排序来选取最优教师。

#### 5.1 选择层级（优先级从高到低）

| 层级 | 条件 | 说明 |
|---|---|---|
| 第一层 | 教师配置了任课学院 且 该班级属于该学院 | 学院偏好教师优先 |
| 第二层 | 教师配置了任课层次 且 该班级属于该层次 | 层次偏好教师优先 |
| 第三层 | 教师关联的教材与班级使用的教材有交集 | 同教材教师优先 |
| 第四层 | 所有剩余候选教师 | 按分数 + 负载排序 |

**核心原则**：配置了任课学院/层次的教师，在其匹配的班级上**始终优先于**未配置偏好的教师，不受负载比较影响。只有在同一层级有多位候选教师时，才按匹配分数和负载排序。

#### 5.2 评分参考（calcMatchScore）
...
同一层级内评分相同时，按**当前负载升序**排列（选最空闲的教师）。
...
排课采用**四阶段分配**策略，每阶段使用 `eligibilityFilter` 限制哪些教师-班级对可以参与：

#### 阶段1：学院偏好匹配

仅允许 `teacher.schedulingCollegeIds` 包含 `cls.collegeId` 的教师-班级对参与分配。

**关键效果**：配置了任课学院的教师在此阶段**只会被分配到匹配的学院班级**，不会被其他学院的班级消耗容量。

#### 阶段2：层次偏好匹配

处理阶段1剩余班级，仅允许 `teacher.schedulingLevelIds` 包含 `cls.trainingLevelId` 的教师-班级对参与。

#### 阶段3：同教材匹配

处理阶段2剩余班级，仅允许教师关联的教材与班级使用的教材有交集的教师-班级对参与。

**关键效果**：确保教师优先安排到使用其熟悉教材的班级，避免跨教材教学。只有在前两个偏好阶段未能覆盖的班级，才进入此阶段按教材匹配分配。

#### 阶段4：常规分配

处理所有剩余班级，不限教师（使用分层选择 + 分数/负载排序）。

#### 阶段内的选择逻辑

每个阶段内部使用 `selectBestTeacher` 分层选择和容量约束，逻辑与前述一致。`assignRound` 接受可选的 `eligibilityFilter(teacher, cls)` 参数，在容量检查之前先过滤教师候选资格。

#### 单阶段内的分配流程（伪代码）

```
assignRound(classList, eligibilityFilter):
  对每个待分配班级：
    1. 筛选候选教师：
       - 资格过滤：eligibilityFilter(teacher, cls) 必须通过（如有设置）
       - 总体容量检查：assignedHours + cls.weeklyHours ≤ 剩余容量（standard 或 full）
       - defaultWeeklyHours 检查（如设置）：已有课时 + 新分配 ≤ defaultWeeklyHours
    2. 分层选择最佳教师（selectBestTeacher）：
       - 第一层：从有学院偏好的候选教师中选择
       - 第二层：从有层次偏好的候选教师中选择
       - 第三层：从有教材匹配的候选教师中选择
       - 第四层：按分数和负载排序选择
    3. 选择排名第一的教师，累加其 assignedHours
    4. 无候选教师 → 该班级进入未分配列表
```
...
三项匹配因子（任课学院、任课层次、同教材）均始终生效，无需额外勾选。

系统保留了 `scheduleConditions` 参数用于未来扩展更多排课偏好。
...
4. **分层选择取代分数排序**：原算法使用简单分数排序（教材+3 > 学院+1 + 层次+1），导致配置了任课学院的教师可能被无偏好的低负载教师挤掉。现已改为分层选择（selectBestTeacher），学院偏好 > 层次偏好 > 教材匹配 > 负载平衡，确保偏好设置始终生效。

5. **无跨课程均衡**：每次排课只针对单个课程独立计算，不会考虑教师在其他课程上的排课情况来做跨课程的均衡分配（虽然 `totalWeeklyHours` 参与了容量计算，但没有跨课程的公平性考量）。

6. **四阶段分配缓解但未完全消除容量消耗问题**：阶段1仅处理学院偏好对，阶段2仅处理层次偏好对，阶段3仅处理同教材匹配对，确保偏好教师的容量不被非偏好班级消耗。但如果某学院/层次/教材的班级总课时远超对应教师的容量，多余班级仍需进入后续阶段由其他教师承担。
