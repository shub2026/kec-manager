...
排课采用**三阶段分配**策略，每阶段使用 `eligibilityFilter` 限制哪些教师-班级对可以参与：
...
6. **三阶段分配缓解但未完全消除容量消耗问题**：阶段1仅处理学院偏好对，阶段2仅处理层次偏好对，确保偏好教师的容量不被非偏好班级消耗。但如果某学院的班级总课时远超偏好教师的容量，多余班级仍需进入阶段3由无偏好教师承担。
