...
**版本** v1.23.0 · **架构** 前后端分离 + 微信小程序 · **部署** Docker / PM2 + Nginx
...
| 统一查询 | 开课查询、教材查询、方案查询、课程查询（按课程聚合各培养方案采用情况与各学期课时，含 Excel 导出）、任课查询（教师/教材双视角）、课时查询（教师历年学期课时汇总与班级明细，含合班数/教材覆盖/Excel 导出），多维度筛选与级联联动 |
...
| 测试 | Vitest + Supertest（后端 81 文件 / 1943 用例，前端 49 文件 / 530 用例） |
...
npm test                 # Vitest（81 个测试文件 / 1943 用例）
...
npm test                 # Vitest（49 个测试文件 / 530 用例）
npm run test:coverage    # 覆盖率报告
npm run lint             # ESLint 检查并修复
npm run format           # Prettier 格式化
```
...
│   ├── pages/                       # 13 个页面（查询/统计/教师与用户管理/访客注册）
│   ├── utils/                       # 请求封装、认证、学期工具
...
所有接口以 `/api` 为前缀，除登录、注册和健康检查外均需 JWT 认证。
...
| 排课 | `/api/teaching-arrange` | 手动/自动/批量排课 + 锁定 + SSE 进度 + 教师历年课时统计 |
