  ...
  function calcTotalHours(course) {
    ...
    for (let s = course.startSemester; s <= course.endSemester; s++) {
      ...
      if (sem) {
        ...
        const weeks = sem.weeksCount || (semesterWeeksRef ? semesterWeeksRef.value[s - 1] : null) || 18
        total += hours * weeks
      } else {
        const hours = course.weeklyHours || 0
        const weeks = course.weeksPerSemester || (semesterWeeksRef ? semesterWeeksRef.value[s - 1] : null) || 18
        total += hours * weeks
      }
    }
