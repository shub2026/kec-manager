<template>
  <div class="filter-bar">
    ...
    <el-select
      ...
      placeholder="入学年份"
      class="filter-narrow"
      :disabled="!filteredEnrollmentYears.length && (localFilters.collegeId || localFilters.majorId || localFilters.trainingLevelId)"
      @change="$emit('change')"
    >
      <el-option v-for="year in filteredEnrollmentYears" :key="year" :label="year + '年'" :value="year" />
    </el-select>
    <el-select
    ...
    <el-select
      ...
      placeholder="培养方案"
      class="filter-medium"
      :disabled="!filteredPlans.length && (localFilters.collegeId || localFilters.majorId || localFilters.trainingLevelId)"
      @change="$emit('change')"
    >
...
function handleCollegeChange() {
  ...
  handleParentChange('collegeId', ['majorId', 'trainingLevelId', 'enrollmentYear', 'planId'], () => emit('change'));
}

// 处理专业变化
function handleMajorChange() {
  // 当专业改变时，清空层次、入学年份和培养方案选择
  handleParentChange('majorId', ['trainingLevelId', 'enrollmentYear', 'planId'], () => emit('change'));
}
